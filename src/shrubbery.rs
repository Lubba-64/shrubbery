use crate::{
    ...
    branch::{Branch2d, Branch3d},
    math_utils::{dist_to_line, dist_to_line_2d, rotate_point},
    shape::{Shape2d, Shape3d},
};

use glam::{ivec2, ivec3, vec2, IVec2, IVec3, Vec2, Vec3};

/// a tree/bush, pressentation that you can grow and modify through post processing effect
pub struct Shrubbery3d {
    pub branches: Vec<Branch3d>,
    pub attractors: Vec<Attractor3d>,
...
/// a tree/bush, pressentation that you can grow and modify through post processing effect
pub struct Shrubbery2d {
    pub branches: Vec<Branch2d>,
    pub attractors: Vec<Attractor2d>,
    pub settings: AlgorithmSettings,
    pub generator_settings: AttractorGeneratorSettings,
    pub min_bounds: Vec2,
    pub max_bounds: Vec2,
...
impl Shrubbery2d {
    pub fn new(
        ...
        Self {
            ...
            settings,
            generator_settings,
            min_bounds: Vec2::splat(0f32),
            max_bounds: Vec2::splat(0f32),
        }
    }

    /// rturns size of the bounding box
    pub fn get_bounding_size(&self) -> IVec2 {
        let (min_bounds, max_bounds) = self.get_bounds();
        max_bounds - min_bounds
    }

    /// return the half size bounds, imagine a quad on the xy axis
    pub fn get_plane_half_size(&self) -> f32 {
        let size = self.get_bounding_size();
        (size.x.max(size.y) as f32 * 0.5).ceil()
    }
    ...
    // expand bounding if branch_pos is outside
    pub fn update_bound(min_bounds: &mut Vec2, max_bounds: &mut Vec2, branch_pos: Vec2) {
        min_bounds.x = min_bounds.x.min(branch_pos.x);
        min_bounds.y = min_bounds.y.min(branch_pos.y);
        max_bounds.x = max_bounds.x.max(branch_pos.x);
        max_bounds.y = max_bounds.y.max(branch_pos.y);
    ...
    /// spawn particles inside provided shape, based on settings
    pub fn spawn_attractors_from_shape<TShape>(&mut self, pos: Vec2, shape: TShape)
    where
        TShape: Shape2d,
    {
        shape.generate(
    ...
    pub fn post_process_gravity(&mut self, gravity: f32) {
        ...
        for branch in self.branches.iter_mut() {
            // branch.
            let branch_plane = vec2(branch.pos.x, branch.pos.y);
            let root = Vec2::ZERO;
            let dist_to_root = branch_plane.distance(root);
    ...
    pub fn post_process_spin(&mut self, spin_amount: f32) {
        let plane_half_size = self.get_plane_half_size();
        for branch in self.branches.iter_mut() {
            let branch_xy = vec2(branch.pos.x, branch.pos.y);
            let root = Vec2::ZERO;
            let dist_to_root = branch_xy.distance(root);
            let weight = dist_to_root / plane_half_size;

            let y_weight = (branch.pos.y * 0.3).cos() * 0.5 + 0.5;

            let new_xz = rotate_point(branch_xy, spin_amount * weight * y_weight);
            branch.pos.x = new_xz.x;
            branch.pos.y = new_xz.y;
        }
    }

    /// returns (distance, index of branch)
    pub fn distance_to_branch(&self, pos: Vec2) -> (f32, usize) {
        let mut closest = f32::MAX;
        let mut index = 0;
        for (i, branch) in self.branches.iter().enumerate() {
            let Some(parent_index) = branch.parent_index else {
            ...
            let d = dist_to_line_2d(pos, self.branches[parent_index].pos, branch.pos);
            if d < closest {
                closest = d;
                index = i;
            }
